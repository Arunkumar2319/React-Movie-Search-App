...
const MovieList = (props) => {
    ...
    const credentialDetails = useSelector((store) => store?.credentials?.credentialDetails)
        ...
        <>
            {props?.movies?.map((movie, index) =>
                <div key={index} className='image-container  d-flex movie-frame justify-content-start p-0 m-3'>
                    ...
                    {credentialDetails != null ? <div className='overlay d-flex align-items-center justify-content'
                        onClick={() => dispatch(addFavouriteMovie(movie))}
                    >
                        <AddToFavourite title={'Add to favourites'} />
                    </div> : null}
                    {favouriteMovieIds?.includes(movie.imdbID) &&  credentialDetails != null ? (
